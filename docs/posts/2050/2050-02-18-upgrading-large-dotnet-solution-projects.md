---
title: "Upgrading Larget .NET Solutions with lots of projects"
categories:
  - Blog
tags:
  - .NET
...
```mermaid
graph TD;
	ps[PARSE SolutionFile Projects]-->lp[Open ProjectFile]-->dpt
	dpt[[Determine ProjectType]]-->updFw
	updFw[Update TargetFramework to new version]-->isAzFn
	isAzFn{Is Azure Function}-- Yes -->upAzFn
	isAzFn-- No --> pPackages
	upAzFn[Update AzureFunctionsVersion to latest]--> pPackages
	
	pPackages[[ProcessPackageReferences]]-->svProj
	
	svProj[Save And Close ProjectFile]-->nxtFile
	nxtFile{Do we have another file in loop}-- Yes -->lp
	nxtFile-- No -->Completed
```
...
```mermaid
graph TD;
	parsePr[PARSE PackageReferences]-->getPrv
	getPrv[Get Current Version of Package] -->findLatestPr
	findLatestPr[Find Latest Version of Package] --> cPr[Cache Latest Version of Package] -->UpdPv
	UpdPv[Update Package Version]--> isNextPr
	isNextPr{Is another Package}-- Yes -->getPrv
	isNextPr-- No --> Completed
```
...
```mermaid
graph TD;
	dc[Define Collection]-->lp[[Get List of Projects]]
	lp-->opf[Open Project File]
	opf-->chf[Check Framework]
	chf -->isOld{Is Framework due for update}
	isOld-- Yes --> adc[Add to Collection]
	isOld-- No --> nxtFile
	adc --> nxtFile
	
	nxtFile{Do we have another file in loop}-- Yes -->opf
	nxtFile-- No -->rtc[Return Collection]
```
...
flowchart TD;
    lp --> of
    mp{Another Project File}-- No -->lc
    subgraph Main
        lp[Loop Through Projects]
        lc[Loop completed]
    end
    subgraph updproj[Update Project File]
        of[Open Project File] --> fr
        fr[Find and Replace Framework String] -->frAzFn
        frAzFn[Find and Replace AzureFunctionsVersion string] -->sf
        sf[Save File] -->mp
        mp{Another Project File}-- Yes -->of
    end
```
...
```mermaid
flowchart TD;
	title[<strong>Update Project File Process</strong>] -->op
	op[Open Project File] -->frtw
	frtw[Find / Replace Target Framework] -->frazFn
	frazFn[Find / Replace Azure Functions Version] -->getpackrefs
	getpackrefs[get packager references list and loop] -->getpn
	getpn[get package name and current version] -->glv
	glv[[get latest package version from nuget repo]] -->updv
	updv[update package version to latest in project] -->anotp
	anotp[do we have another packagereference]-- Yes -->getpn
	anotp-- No -->endp
	endp["completed - save project file updates"]
```
...
```mermaid
flowchart TD;
	title[<strong>Fetch latest nuget package version V1</strong>] -->getlvcache
	getlvcache{Is latest version in cache}
	getlvcache--> Yes --> retcv[Get latest version from cache] -->retv
	getlvcache--> No --> getfromapi
	getfromapi[Get latest version from remote resource] -->addc[Add to cache] -->retv
	retv[return latest version]
...
```mermaid
flowchart TD;
	title[<strong>Fetch latest nuget package version V2</strong>] -->trygetfromcach
	trygetfromcach[Try Get latest version from cache] -->isCacheResult
	isCacheResult{Is there a cache result}-- Yes --> retv
	isCacheResult--> No --> getfromapi
	getfromapi[Get latest version from remote resource] -->addc[Add to cache] -->retv
	retv[return latest version]
